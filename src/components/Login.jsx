...
/**
 * Login.jsx — FitZone premium authentication screen
 * ---------------------------------------------------
 * Drop-in replacement for the existing Login page.
 * All original logic is preserved unchanged: the fetch call, JWT storage,
 * navigation, validation condition/messages, and toast notifications.
 * Everything else here is presentation only.
 *
 * New dependencies to install (the original file only used react-router-dom
 * and react-toastify, both already in your project):
...
/* Lightweight count-up used by the floating stat badges */
...
/* The page's signature element: a literal heartbeat trace running down the
   seam between the brand story (left) and the access point (right). */
...
const FloatingInput = ({ id, type, name, value, onChange, label, icon, error, rightSlot }) => (
  <div>
...
const Login = () => {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-hidden bg-[#0B0B0D] md:flex-row">
      ...
      {/* heartbeat seam — the page's signature element, only relevant once
          the split layout exists at md+ */}
        ...
        <div className="relative z-10 flex w-full flex-col justify-between p-10 lg:p-14">
          ...
          >
            ...
            <div>
              ...
              <p className="ff-mono text-[10px] uppercase tracking-[0.2em] text-gray-400">
          ...
          <div className="max-w-xl">
            ...
            >
              Join thousands of members achieving their fitness goals with expert trainers and personalized programs.
            </motion.p>
          </div>
      ...
      <div className="relative z-[2] flex w-full flex-1 items-center justify-center px-4 py-10 sm:px-6 md:bg-gradient-to-br md:from-[#111113] md:to-[#0B0B0D] lg:px-12">
        ...
        >
          ...
          <h2 className="ff-display text-2xl text-white sm:text-3xl">WELCOME BACK</h2>
          <p className="ff-body mt-2 text-sm text-gray-400">Sign in to continue your fitness journey.</p>
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex cursor-pointer items-center gap-2 text-sm text-gray-300">
                <input
                  ...
                  className="h-4 w-4 rounded border-white/30 bg-white/10 accent-[#FF3B30] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF3B30]/50"
                />
                Remember me
              </label>
              <Link to="/forgot-password" className="text-sm font-medium text-gray-300 transition-colors hover:text-[#FF3B30]">
                Forgot password?
              </Link>
            </div>
          ...
          <p className="ff-body mt-7 text-center text-sm text-gray-400">
            New to FitZone?{" "}
            <Link to="/signup" className="font-semibold text-white underline-offset-4 hover:text-[#FF3B30] hover:underline">
              Create Account
            </Link>
          </p>

          <div className="mt-7 flex items-center justify-center gap-5 border-t border-white/10 pt-5">
            <span className="flex items-center gap-1.5 text-[11px] text-gray-500">
              <ShieldCheck className="h-3.5 w-3.5 text-[#C9A24B]" /> Secure Login
            </span>
            <span className="flex items-center gap-1.5 text-[11px] text-gray-500">
              <BadgeCheck className="h-3.5 w-3.5 text-[#C9A24B]" /> 10K+ Trust Us
            </span>
            <span className="flex items-center gap-1.5 text-[11px] text-gray-500">
